...
import re
content = re.sub(
    r'def test_crm_proxy_intelligent_deduplication\(self\):.*?def test_crm_proxy_timeline_merge',
    """def test_crm_proxy_intelligent_deduplication(self):
            ...
            def read(self):
                payload = {
                    "leads": [
                        ...
                        {"id": 3, "email": "dup@example.com", "phone": "777", "signup_count": 1},
        ...
        self.assertEqual(dup_lead["id"], 3) # Because ID 3 is the highest among duplicates!
        self.assertEqual(dup_lead["signup_count"], 4) # 1 + 2 + 1
        
        self.assertEqual(unique_lead["id"], 4)
        self.assertEqual(unique_lead["signup_count"], 1)

    def test_crm_proxy_timeline_merge""",
    content,
    flags=re.DOTALL
)
